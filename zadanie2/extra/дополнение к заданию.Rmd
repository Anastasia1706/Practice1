...
Сравним MSE, полученные в первом и втором случаях. Сгенерируем обе подвыборки, подсчитаем ошибки на обучающей и тестовой выборках и сведем результаты в таблицу.
...
В этом блоке будет представлен расчет модельных значений для первой и второй модели.
...
# степени свободы у наименьшей ошибки на обучающей выборке
min.MSE.table <- min(MSE.table$MSE.TRAIN)

kable(MSE.table)
```

Из таблицы видно, что наименьшое значение MSE на тестовой выборке имеет модель 2 (res.sd=2), но у модели 1 существенно ниже MSE на обучающей выборке. Так как ошибки на тестовой выборке сильно не отличаются друг от друга, то есть смысл предпочесть модель 1.